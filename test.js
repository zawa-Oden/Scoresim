    ...
    function frontsave(){
        for(let i = 1 ; i < 7 ; i++){
            for(let j = 1 ; j < 6 ; j++){
                ...
                baisavearr[i-1][j-1] = document.getElementById("setBai"+i+j).value;
            }
        }
        cookies1 = name + '=' + JSON.stringify(frontsavearr) + ';';
        cookies2 = name + '=' + JSON.stringify(baisavearr) + ';';
        period = 100; //保存日数
        expire = new Date();
        expire.setTime(expire.getTime() + 1000 * 3600 * 24 * period);
        expire.toUTCString();
        cookies1 += 'expires=' + expire + ';';
        cookies2 += 'expires=' + expire + ';';
